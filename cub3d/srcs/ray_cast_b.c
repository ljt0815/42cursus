...
/*   Updated: 2021/05/10 16:17:53 by jitlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_init(t_dat *dat)
{
	...
	dat->texture = ft_calloc(7, sizeof(int *));
	if (dat->texture == 0)
		err_msg("memory allocate error");
	i = -1;
	while (++i < 7)
...
void	load_texture(t_dat *dat)
{
	...
	load_image(dat, dat->texture[5], "textures/item2.xpm", &img);
	load_image(dat, dat->texture[6], "textures/item3.xpm", &img);
}
